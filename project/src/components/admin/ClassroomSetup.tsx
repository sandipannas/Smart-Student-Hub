...
interface Classroom {
  ...
  faculty: string;
  image: string;
  seats: Seat[]; // you can refine this later
  createdAt: string;
  updatedAt?: string;
}
...
export default function ClassroomSetup() {
  ...
  const saveClassrooms = (updatedClassrooms: Classroom[]) => {
  ...
  if (!files || files.length === 0) return; // exit if no file selected
  ...
  const saveSeatLayout = (seats:any[]) => {
