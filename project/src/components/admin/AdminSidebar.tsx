...
const AdminSidebar: React.FC<AdminSidebarProps> = ({setLoad}) => {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  
  const menuItems = [
    { id: "overview", label: "Dashboard", icon: Home },
    { id: "profile", label: "Profile", icon: User },
    { id: "classsetup", label: "Classroom Setup", icon: SettingsIcon },
    { id: "accreditation", label: "Accreditation Reports", icon: FileText },
    { id: "analytics", label: "Institution Analytics", icon: BarChart3 },
    { id: "system", label: "System Management", icon: Settings },
    { id: "register", label: "Register Faculty", icon: UserPlus },
  ];
  ...
  return (
    <>
      ...
      {/* Admin Info */}
      <NavLink
        className={`p-4 sm:p-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-blue-50
      ...
      >
        <div className="flex flex-col items-center justify-center ">
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-600 rounded-full text-white flex items-center justify-center mb-4">
            {user?.profilePic ? (
              <img
                src={user.profilePic}
                alt={user?.name[0]}
